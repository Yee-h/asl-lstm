...
def compare_hdf5_structure(official_path: str, processed_path: str) -> bool:
    ...
    with h5py.File(official_path, 'r') as f_official, \
        ...
        if official_count == processed_count:
            ...
            print(f"  ✗ 样本数量不一致: 官方 {official_count}, 预处理 {processed_count}")
            is_consistent = False
...
def main():
    ...
    official_dir = os.path.join(cfg.PROJECT_ROOT, f"WLASL{subset}")
    ...
    if os.path.exists(processed_dir):
        ...
        # 对比标签映射文件
        official_maplabels = os.path.join(official_dir, f"wlasl_{subset}_maplabels.json")
        processed_maplabels = os.path.join(processed_dir, f"wlasl_{subset}_maplabels.json")
        ...
        for split in splits:
            official_hdf5 = os.path.join(official_dir, f"WLASL{subset}_135-{split}.hdf5")
            processed_hdf5 = os.path.join(processed_dir, f"WLASL{subset}_135-{split}.hdf5")
