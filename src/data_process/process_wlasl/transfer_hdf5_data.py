...
def get_data_dir(dataset_scale: str) -> Path:
    ...
    return base_dir / "pose_action_dataset" / f"WLASL{dataset_scale}"
    ...
    for dataset_name in source_group.keys():
        ...
        if isinstance(source_dataset, h5py.Dataset):
            ...
            if dataset_name == "video_name":
                ...
                # 更新子集划分
                updated_video_name = update_video_name(video_name, source_split, target_split)
...
def verify_transfer(dataset_scale: str, target_split: str, sample_keys: list[str], verbose: bool = True) -> bool:
    """
    验证迁移后的数据完整性
    ...
    with h5py.File(target_path, "r") as f:
        ...
        for key in keys_to_check:  # 只验证前几个样本
            ...
            group = f[key]
            required_fields = ["data", "label", "video_name", "height", "width"]
            
            for field in required_fields:
                if field not in group:
                    issues.append(f"样本 '{key}' 缺少字段 '{field}'")
            ...
            if "video_name" in group:
                ...
                target_lower = target_split.lower()
                if f"/{target_lower}/" not in video_name:
                    issues.append(f"样本 '{key}' 的 video_name 未正确更新: {video_name}")
...
def main() -> None:
    ...
    # 显示数据集信息
    show_dataset_info(config.DATASET_SCALE)
    ...
    result = transfer_data(
        ...
        transfer_percentage=config.TRANSFER_PERCENTAGE,
        delete_from_source=config.DELETE_FROM_SOURCE,
