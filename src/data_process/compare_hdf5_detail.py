...
def compare_data_statistics(split: str):
    ...
    with h5py.File(official_path, "r") as f_o, h5py.File(processed_path, "r") as f_p:
        ...
        print(
            f"  预处理 - 总帧数: {sum(p_frame_counts)}, 平均: {np.mean(p_frame_counts):.1f}, 最小: {min(p_frame_counts)}, 最大: {max(p_frame_counts)}"
        )
        ...
        print(
            f"  预处理 - mean: {p_concat.mean():.6f}, std: {p_concat.std():.6f}, min: {p_concat.min():.6f}, max: {p_concat.max():.6f}"
        )
