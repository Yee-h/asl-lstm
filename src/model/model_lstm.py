...
class Attention(nn.Module):
    ...
    """
    def __init__(self, hidden_dim, attention_dim=cfg.ATTENTION_DIM):
        """
        初始化注意力层。
...
class BiLSTMAttention(nn.Module):
    ...
    """
    def __init__(self, input_size=cfg.INPUT_SIZE, hidden_size=cfg.HIDDEN_SIZE, 
                 num_layers=cfg.NUM_LAYERS, num_classes=cfg.NUM_CLASSES, 
                 dropout=cfg.DROPOUT, attention_dim=cfg.ATTENTION_DIM):
        """
        初始化模型层。
    ...
    def forward(self, x, lengths):
        ...
        mask = torch.arange(seq_len, device=x.device).unsqueeze(0) < lengths.unsqueeze(1).to(x.device)
...
def get_model(use_attention=cfg.USE_ATTENTION):
    """
    根据配置返回相应的模型实例。
