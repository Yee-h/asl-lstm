...
## [2026-02-21 19:05:14] E02-T2 完成（延长收敛窗口）
- **实验配置**:
  - 训练：`uv run python src/model/train_lstm.py --run-tag exp_e02_t2_ep120 --seed 42 --epochs 120`
  - 评估：`uv run python src/model/evaluate_lstm.py --model-path src/checkpoints/exp_e02_t2_ep120/best_model.pth`
- **结果**:
  - 训练集准确率：`75.52%`
  - 验证集准确率：`68.25%`
  - 测试集准确率：`60.47%`（`logs/evaluation_report_20260221_190514.txt`）
- **结论**:
  - 相比 `E02-T1`（val `55.79%` / test `47.29%`）有显著恢复，说明收敛窗口拉长有效。
  - 但仍低于历史最优基线（val `73.29%` / test `69.38%`），尚未满足 E02 成功门槛。
  - 下一步执行 `E02-T3`，在保留较长训练窗口前提下做单变量微调（优先学习率或权重衰减）。

## [2026-02-21 19:54:51] E02-T3 完成（超长窗口验证）
- **实验配置**:
  - 训练：`uv run python src/model/train_lstm.py --run-tag exp_e02_t3_ep240 --seed 42 --epochs 240`
  - 评估：`uv run python src/model/evaluate_lstm.py --model-path src/checkpoints/exp_e02_t3_ep240/best_model.pth`
- **结果**:
  - 训练集准确率：`80.03%`
  - 验证集准确率：`68.84%`（峰值）
  - 测试集准确率：`64.34%`（`logs/evaluation_report_20260221_195451.txt`）
- **结论**:
