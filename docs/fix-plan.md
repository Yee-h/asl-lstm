...
### E04（进行中）数据质量阈值与采样策略实验
- 主因子：`min_valid_ratio_per_sample` 与 `use_weighted_sampler/sampler_power`。
- 目标：提升长尾类别召回，优先提高测试集准确率。
- 已执行子实验：
  - `E04-T1`（`exp_e04_t1_mvr040_ls001_ep240`，`min_valid_ratio_per_sample=0.40` + `label_smoothing=0.01`）
    - 训练结果：最佳 `train=79.82%`、最佳 `val=73.89%`（epoch 237，`tool_c83e10bd4001S1Bt5KyEJoduhr`）。
    - 测试结果（no-TTA）：`67.05%`（`logs/evaluation_report_20260222_134528.txt`）。
    - 结论：与 `E03-T1` 几乎完全一致；当前数据集在加载阶段样本量不变（train/val/test: `1442/337/258`），说明阈值 `0.35->0.40` 未形成有效筛选。
  - `E04-T2`（`exp_e04_t2_ws070_ls001_ep240`，`use_weighted_sampler=True` + `sampler_power=0.70` + `label_smoothing=0.01`）
    - 训练结果：最佳 `train=76.56%`、最佳 `val=70.33%`（epoch 228，`tool_c841907b90017D2LLsG6eJ1C2y`）。
    - 测试结果（no-TTA）：`65.50%`（`logs/evaluation_report_20260222_144634.txt`）。
    - 结论：较 `E03-T1` 明显回退，当前类别重采样配置不优。
- 阶段中间结论：E04 已验证两条路径（轻度质量阈值抬升、历史采样指数复用）均未带来 test 改善，下一步需扩大阈值跨度或继续扫描 `sampler_power`。
...
- [ ] E04：进行中（已完成 `E04-T1/T2`，待补充更大阈值跨度或采样指数扫描）。
- [ ] E05：待执行。
- [ ] E06：待执行。
